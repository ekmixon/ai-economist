...
class BaseEnvironment(ABC):
    ...
    def metrics(self):
        ...
        for component in self._components:
            ...
            for k, v in m_metrics.items():
                metrics[f"{component.shorthand}/{k}"] = v
    ...
    def get_component(self, component_name):
        ...
        if component_name not in self._components_dict:
            if component_name not in self._shorthand_lookup:
                raise KeyError(
                    (
                        f"No component with name or shorthand name {component_name} found; registered components are:\n"
    ...
    def get_agent(self, agent_idx):
        ...
        if agent is None:
            raise ValueError(f"No agent with associated index {agent_idx}")
        return agent
    ...
    def _build_packager(sub_obs, put_in_both=None):
        ...
        for k, v in sub_obs.items():
            multi_d_array = len(v.shape) > 1 if isinstance(v, np.ndarray) else False
    ...
    def _generate_observations(self, flatten_observations=False, flatten_masks=False):
        ...
        agent_wise_planner_obs = {
            f"p{str(agent.idx)}": {} for agent in self.world.agents
        }
        ...
        for idx, o in world_obs.items():
            if idx in obs:
                obs[idx].update({f"world-{k}": v for k, v in o.items()})
                ...
                agent_wise_planner_obs[idx].update({f"world-{k}": v for k, v in o.items()})
            else:
                raise KeyError
        ...
        for component in self._components:
            for idx, o in component.obs().items():
                if idx in obs:
                    obs[idx].update({f"{component.name}-{k}": v for k, v in o.items()})
                elif idx in agent_wise_planner_obs:
                    agent_wise_planner_obs[idx].update(
                        {f"{component.name}-{k}": v for k, v in o.items()}
                    )
    ...
    def _generate_masks(self, flatten_masks=True):
            ...
            for idx, mask in component_masks.items():
                if isinstance(mask, dict):
                    for sub_action, sub_mask in mask.items():
                        masks[idx][f"{component.name}.{sub_action}"] = sub_mask
                else:
        ...
        if flatten_masks:
            if self.collate_agent_step_and_reset_data:
                flattened_masks = {}
                for agent_id, mask_dict in masks.items():
                    if agent_id == "a":
        ...
        for component in self._components:
            ...
            if isinstance(component_log, dict):
                for k, v in component_log.items():
                    self._dense_log[f"{component.shorthand}-{k}"] = v
